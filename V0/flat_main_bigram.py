...
parser.add_argument('--data_create',default='yes',choices=['yes','no'])
...
if args.data_create == 'yes':     
    ...
    with open('datasets.pkl','wb') as pw:
        pickle.dump(datasets,pw)
    with open('vocabs.pkl','wb') as pw:
        pickle.dump(vocabs,pw)
    with open('embeddings.pkl','wb') as pw:
        pickle.dump(embeddings,pw)

else:
    with open('datasets.pkl','rb') as pw:
        datasets = pickle.load(pw)
    with open('vocabs.pkl','rb') as pw:
        vocabs = pickle.load(pw)
    with open('embeddings.pkl','rb') as pw:
        embeddings = pickle.load(pw)

#print('lattice_vocab:{}\n{}'.format(len(vocabs['lattice']),vocabs['lattice'].idx2word))
...
for k, v in datasets.items():
    ...
    if args.lattice:
        ...
        v.set_input('lattice','seq_len','target')
