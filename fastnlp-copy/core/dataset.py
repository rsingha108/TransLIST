...
setting = "sighum-ngram" ## "sighum-ngram" or "sighum-shr" or "hack-ngram" or "hack-shr" 
    ...
    def apply_field(self, func, field_name, new_field_name=None, **kwargs):
        ...
        if(hasattr(func,'keywords')):
            if(list(func.keywords.keys())[0]=='w_trie'):
                ...
                extra_flag=1
                ignore_count=0
                #with open('/data/nrao/nlp/not_matched.txt') as f:
            ...
            for idx, ins in tqdm(enumerate(self._inner_iter())):
                if(extra_flag):
                    ...
                    if(os.path.exists(path_to_lattice)):
                        ...
                        #temp1 = [[t[0],t[1],str(t[2])] for t in temp1]
                        extra_conn_temp = temp1 # list of list that contain tuples like [s,e,word] for a sentence <-- coming from lattice files
                        extra_conn_final = func(ins[field_name]) # list of list with tuples [s,e,word] <-- coming from lexicon
                        for con_id in range(int(1*len(extra_conn_temp))):
                           extra_conn_final.append(extra_conn_temp[con_id])
                        extra_conn_final = [list(x) for x in set(tuple(x) for x in extra_conn_final)] ## lexicon + lattice files : combined and flattened
                       	results.append(extra_conn_final)
                        # extra_conn_final = []
                        #if big_data :
                        #  if idx < 100000 : results0.append(extra_conn_final)
                        #  elif idx >= 100000 and idx < 200000 : results1.append(extra_conn_final)
                        #  elif idx >= 200000 and idx < 300000 : results2.append(extra_conn_final)
                        #  elif idx >= 300000 and idx < 400000 : results3.append(extra_conn_final)
                        #  elif idx >= 400000 : results4.append(extra_conn_final)
                        #else:
                        #  results.append(extra_conn_final) ## results is a list of list of list : every element is a double list that contains lattices for a sentence. each element of a double list is a tuple [start_index, end_index, word]
                        
                    else:
                        extra_conn_temp = func(ins[field_name])
                        extra_conn_final = extra_conn_temp 
                        results.append(extra_conn_final)
                        #extra_conn_final = []
                        #if big_data :
                        #  if idx < 100000 : results0.append(extra_conn_final)
